#ifndef OCELOT_CRYPTO_HPP
#define OCELOT_CRYPTO_HPP
#include <functional>
#include <memory>
...
namespace crypto {
    ...
    /// Session key material.  This used to come from a global mt19937 seeded
    /// with time(nullptr), which makes every key of a given second guessable;
    /// OpenSSL's CSPRNG is already linked in, so use it.
    inline std::string random_string(const size_t length) {
        std::string str(length, 0);
        if (RAND_bytes(reinterpret_cast<byte *>(str.data()), static_cast<int>(length)) != 1) {
    ...
    public:
        ...
        void fromPublicKey(const std::string &key) override {
            ...
            if (BIO_write(bio, key.data(), static_cast<int>(key.size())) != static_cast<int>(key.size())) {
                BIO_free(bio);
